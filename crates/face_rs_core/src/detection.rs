...
// ── Output parsing ────────────────────────────────────────────────────────────
//
// SCRFD-500M exports 9 named outputs for strides 8, 16, 32:
//   score_{stride}   flat [H*W*2]
//   bbox_{stride}    flat [H*W*2 * 4]
//   kps_{stride}     flat [H*W*2 * 10]
...
fn parse_scrfd_outputs(
    ...
    for &stride in &[8u32, 16, 32] {
        let scores = extract_flat(outputs, &format!("score_{stride}"))?;
        let bboxes = extract_flat(outputs, &format!("bbox_{stride}"))?;
        let kps = extract_flat(outputs, &format!("kps_{stride}"))?;
...
fn extract_flat(outputs: &SessionOutputs, name: &str) -> Result<Vec<f32>> {
    let output = outputs
        .get(name)
        .ok_or_else(|| FaceRsError::Detection(format!("missing output '{name}'")))?;
