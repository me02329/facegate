...
impl V4lCamera {
    pub fn open(device: &str, width: u32, height: u32, fps: u32) -> Result<Self> {
        ...
        // Try MJPEG first (smaller buffers, better FPS on most webcams)
        fmt.fourcc = FourCC::new(b"MJPG");
        let negotiated = dev.set_format(&fmt).unwrap_or_else(|_| {
            fmt.fourcc = FourCC::new(b"YUYV");
            dev.set_format(&fmt).unwrap_or(fmt)
        });

        let capture_format = if negotiated.fourcc == FourCC::new(b"MJPG") {
            CaptureFormat::Mjpeg
        } else {
            CaptureFormat::Yuyv
        };
