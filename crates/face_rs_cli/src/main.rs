...
use face_rs_core::config::{Config, DEFAULT_CONFIG_PATH};
use tracing_subscriber::EnvFilter;

#[derive(Debug, Parser)]
#[command(
    name = "face-rs",
    about = "Facial authentication tool for Linux\n\nRun without arguments to open the interactive menu.",
    version
)]
struct Cli {
...
enum Command {
    ...
    CameraTest { #[arg(long)] device: Option<String> },
    /// Enroll a face for a user (requires root)
    Add { username: String, #[arg(long)] label: Option<String> },
...
fn main() {
    ...
    if let Some(Command::Completions { shell }) = cli.command {
        generate(shell, &mut Cli::command(), "face-rs", &mut std::io::stdout());
        return;
    }
    ...
    let config = match Config::load(&cli.config) {
        ...
        Err(e) => {
            if matches!(cli.command, Some(Command::Auth { .. })) {
                ...
                std::process::exit(face_rs_core::error::AuthExitCode::ConfigError as i32);
            }
        ...
        None => {
            ...
            loop {
                match tui::main_menu::run(&config) {
                    Err(e)     => { eprintln!("TUI error: {e}"); return; }
                    Ok(false)  => return,
                    Ok(true)   => {
                        // User selected Configure → open config TUI then re-enter menu
                        let _ = commands::configure::run(config.clone(), config_path.clone());
                    }
                }
            }
        }
...
fn run_command(cmd: Command, config: Config, config_path: std::path::PathBuf) -> anyhow::Result<()> {
    match cmd {
        ...
        Command::Add { username, label } => commands::add::run(&config, &username, label.as_deref()),
