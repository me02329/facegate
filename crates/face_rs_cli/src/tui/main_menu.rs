...
enum Action { Configure, Doctor, CameraTest, Add, List, Test }
...
fn build_items() -> Vec<MenuItem> {
    vec![
        MenuItem { icon: "⚙ ", label: "Configure",       description: "Edit settings",                          action: Some(Action::Configure),  needs_user: false },
        MenuItem { icon: "✓ ", label: "Doctor",           description: "Check installation status",              action: Some(Action::Doctor),     needs_user: false },
        MenuItem { icon: "◉ ", label: "Camera Test",      description: "Test camera and face detection",         action: Some(Action::CameraTest), needs_user: false },
        MenuItem { icon: "+ ", label: "Enroll Face",      description: "Add a new face template  (root)",        action: Some(Action::Add),        needs_user: true  },
        MenuItem { icon: "= ", label: "List Templates",   description: "View enrolled templates",                action: Some(Action::List),       needs_user: true  },
        MenuItem { icon: "~ ", label: "Test Recognition", description: "Live recognition test      (root)",      action: Some(Action::Test),       needs_user: true  },
        MenuItem { icon: "  ", label: "---",              description: "",                                        action: None,                     needs_user: false },
        MenuItem { icon: "x ", label: "Quit",             description: "Exit face-rs",                           action: None,                     needs_user: false },
    ]
}
...
#[derive(PartialEq)]
enum InputMode { Menu, UsernameInput }

enum PanelState {
    /// Idle — show welcome text.
    Idle,
    /// A command is running in a background thread.
    Running { rx: Receiver<String>, lines: Vec<String>, tick: u8 },
    /// Command finished — show full output, wait for Enter.
    Done { lines: Vec<String>, scroll: usize, had_error: bool },
}
...
impl<'a> App<'a> {
    ...
    fn is_sep(&self, i: usize) -> bool { self.items[i].label == "---" }
    fn is_quit(&self, i: usize) -> bool { self.items[i].label == "Quit" }

    fn move_up(&mut self) {
        loop {
            self.selected = if self.selected == 0 { self.items.len() - 1 } else { self.selected - 1 };
            if !self.is_sep(self.selected) { break; }
        }
    }
    ...
    fn enter_menu(&mut self) {
        if self.is_quit(self.selected) { self.should_quit = true; return; }
    ...
    fn launch(&mut self, action: Action, username: Option<String>) {
        ...
        thread::spawn(move || {
            let result = match &action {
                ...
                Action::Add        => commands::add::run_streaming(&config, username.as_deref(), None, &tx),
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Err(TryRecvError::Disconnected) => {
                        ...
                        let had_error = done_lines.iter().any(|l| l.starts_with("\nError:") || l.starts_with("Error:"));
                        self.panel = PanelState::Done { lines: done_lines, scroll: 0, had_error };
                        return;
                    }
                }
            }
...
fn event_loop<B: ratatui::backend::Backend>(terminal: &mut Terminal<B>, app: &mut App) -> Result<bool> {
    loop {
        terminal.draw(|f| render(f, app))?;
        app.poll_channel();

        if app.should_quit  { return Ok(false); }
        if app.open_config  { return Ok(true);  }
...
fn handle_key(app: &mut App, code: KeyCode) {
    match app.input_mode {
        InputMode::UsernameInput => match code {
            ...
            KeyCode::Backspace => { app.username_buf.pop(); }
...
fn render_header(f: &mut Frame, area: Rect) {
    let mut lines: Vec<Line> = LOGO.iter().map(|row| {
        Line::from(Span::styled(*row, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)))
    }).collect();
...
fn render_menu(f: &mut Frame, area: Rect, app: &App) {
    ...
    let border_color = if panel_active { Color::Cyan } else { Color::DarkGray };
    ...
    let items: Vec<ListItem> = app.items.iter().enumerate().map(|(i, item)| {
        ...
        let (icon_s, label_s) = if sel {
            ...
            let hl_b   = Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD);
            (hl, hl_b)
        } else {
...
fn render_panel(f: &mut Frame, area: Rect, app: &App) {
    let (title, border_color, content_lines, hint) = match &app.panel {
        ...
        PanelState::Running { lines, tick, .. } => {
            ...
            let mut out: Vec<Line> = lines.iter()
        ...
        PanelState::Done { lines, scroll, had_error } => {
...
fn render_username_popup(f: &mut Frame, app: &App) {
    ...
    f.render_widget(
        Paragraph::new("  Enter the username:")
            .style(Style::default().fg(Color::DarkGray)),
        layout[1],
    );
    f.render_widget(
