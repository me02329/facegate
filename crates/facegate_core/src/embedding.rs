...
mod tests {
    ...
    #[test]
    fn warp_affine_inverse_handles_rotation() {
        let img = ImageBuffer::from_fn(3, 3, |x, y| image::Rgb([(x + 10 * y) as u8, 0, 0]));

        // x' = -y + 2, y' = x rotates the source 90 degrees clockwise into
        // the output frame. The inverse sampler must map output pixels back
        // to the original source coordinates.
        let out = warp_affine(&img, 0.0, 1.0, 2.0, 0.0, 3, 3);
