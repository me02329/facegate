...
//!
//! This module delegates all heavy work to `face-rs auth --user <name>`,
...
/// face-rs auth exit codes (must match face_rs_core::error::AuthExitCode)
...
fn run_auth_helper(username: &str) -> Result<c_int, ()> {
    let mut child = Command::new("/usr/bin/face-rs")
